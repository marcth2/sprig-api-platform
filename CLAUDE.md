...
- **Testing philosophy — automated vs. manual verification scope:** unit and feature tests own functional/business-logic correctness, whatever a given domain's internal architecture turns out to be — that's what `composer test`'s 100%-coverage gate enforces on every push. A wayfinder's manual verification issue is not a second pass at that same correctness; its job is verifying the real-environment integration seams automated tests structurally can't reach (a real external dependency actually responding, a real queue worker actually consuming a job, a migration actually applying cleanly against real data), plus a targeted spot-check of any shared/cross-cutting code the wayfinder touched. A domain with little internal business logic (e.g. HealthCheck) leans almost entirely on this real-environment check since there isn't much else to verify; a domain with real business logic leans on its automated suite for logic correctness and reserves the verification issue for the seams outside that suite's reach

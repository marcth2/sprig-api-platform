...
- **Wayfinder → QA → release sequencing:** a wayfinder (a "Map" issue tracking a checklist of tickets, e.g. #14) does not close when its tickets close — completing tickets triggers verification, and verification gates the close. Order: (1) all tickets closed, (2) open a QA issue and add it to the wayfinder's own checklist (so the link is GitHub-native and discoverable from either issue, not just documented in prose) — anchor the QA issue to the commit SHA that closed the last ticket, with a manual verification checklist and comments showing the actual command run + actual output for each check, good faith effort not foolproof, real evidence over a rubber-stamp, (3) once the QA issue closes clean, close the wayfinder itself, (4) only then cut the release. A release timestamped before the QA issue that verifies it even exists is a process defect, not a technicality — the wayfinder stays open specifically to prevent that ordering. The minor-vs-major choice is judged by the wayfinder's actual compatibility impact (additive → minor, breaking → major) — it is not automatic just because a wayfinder finished. Patches are reserved for out-of-band fixes between wayfinders (e.g. v0.1.1) and are never tied to a wayfinder completion. While pre-1.0, treat "minor" as "a wayfinder's worth of change," not a stability promise — reserve an actual `1.0.0` bump for a deliberate, separate decision that the API is a stable public contract. The `CHANGELOG.md` entry for the release references the QA issue number for traceability
...
_Last updated: 2026-08-18 (Revised sequencing so the QA issue gates the wayfinder's own close, not just the release — corrects the v0.1.1/#14/#34 ordering gap)_
